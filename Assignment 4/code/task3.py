...
class Encoder(tf.keras.Model):
    ...
    def call(self, x):
        # Input is input language tokens -> (batch_size, seq length)
        x = self.embedding_layer(x)             # -> (batch_size, seq length, embedding_dim)
        _, enc_h, enc_c = self.lstm_layer(x)    # -> (batch_size, embedding_dim) each
...
class Decoder(tf.keras.Model):
    ...
    def call(self, decoder_inputs):
        ...
        init_states = [dec_h, dec_c]    # New hidden states

        out = tf.reshape(dec_out, (-1, dec_out.shape[2]))  # -> (batch_size, out vocab_size)
        out = self.fc_layer(out) # -> (batch_size, out vocab_size)

        return out, init_states
...
def embed_init(shape, dtype=None):
    ...
    # This intializer function is specific to Tensorflow
...
def learning_step(enc_in, dec_trg):
    ...
    with tf.GradientTape() as tape:
        ...
        enc_states = encoder_model(enc_in)
        ...
        for t in range(1, maxlen_hi-1):
            ...
            preds, dec_states = decoder_model(
                [dec_in, dec_states]
            )
            # Compute loss
            loss += compute_loss(
...
if __name__ == "__main__":
    ...
    # Preserve only those sentences which are at most 30 words long
    ...
    # Tokenize lines
    # Need to include words from development set also
    # ... to facilitate translation during evaluation
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            # Peform learning step and record loss
