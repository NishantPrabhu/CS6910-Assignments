...
# ================================================================== #
#  Pretrained VGGNet for image classification                        #
#                                                                    #
...
NUM_CLASSES = 7
vgg_net.classifier = torch.nn.Sequential(
                        torch.nn.Linear(25088, NUM_CLASSES),
                        torch.nn.LogSoftmax(dim=1)
                    )


# Train and testing functions
# Will be called in every epoch
...
def validate(args, model, test_loader, loss_function, epoch, mode='valid'):
    ...
    if mode == 'valid':
        ...
        print("Top 1 accuracy: {:.2f}".format(top_1_accuracy))
...
for epoch in range(epochs):
    ...
    train_loss, train_acc1, train_acc3 = train(args, vgg_net, dataloaders['train'],
                                               loss_function, optimizer, epoch)
    # Test the model and store loss and accuracies
    val_loss, val_acc1, val_acc3 = validate(args, vgg_net, dataloaders['valid'],
                                            loss_function, epoch)
    ...
    if (epoch % save_interval == 0) or (epoch == epochs-1):
        torch.save(vgg_net.state_dict(), save_path + '/vgg_epoch_{}'.format(epoch))
...
test_loss, test_acc1, test_acc3 = validate(args, vgg_net, dataloaders['test'],
...
plt.legend()
plt.savefig('../plots/vgg_loss_history.png')
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('../plots/vgg_top1_accuracy.png')
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('../plots/vgg_top3_accuracy.png')
plt.show()
