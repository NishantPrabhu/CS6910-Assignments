...
for epoch in range(epochs):
    ...
    train_loss, train_acc1, train_acc3 = train(args, googlenet, dataloaders['train'],
                                               loss_function, optimizer, epoch)
    # Test the model and store loss and accuracies
    val_loss, val_acc1, val_acc3 = validate(args, googlenet, dataloaders['valid'],
                                            loss_function, epoch)
    ...
    if (epoch % save_interval == 0) or (epoch == epochs-1):
        torch.save(googlenet.state_dict(), save_path + '/googlenet_epoch_{}'.format(epoch))
...
test_loss, test_acc1, test_acc3 = validate(args, googlenet, dataloaders['test'],
...
plt.legend()
plt.savefig('../plots/googlenet_loss_history.png')
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('../plots/googlenet_top1_accuracy.png')
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('../plots/googlenet_top3_accuracy.png')
plt.show()
